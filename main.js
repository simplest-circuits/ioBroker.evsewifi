...
class Evsewifi extends utils.Adapter {
    ...
    async onReady() {
        ...
        await this.setObjectNotExistsAsync('setCurrent', {
            ...
            native: {},
		        ...
		        await this.setObjectNotExistsAsync('setNumPhases', {
            type: 'state',
            common: {
                name: 'setNumPhases',
                type: 'number',
	...
	async setNumPhases(phases){
      const url = 'http://' + this.config.ip + '/setNumPhases?numPhases='+phases;
      const self = this;
      request({method: "GET", url}, function (error, response, result) {
          if(!error && response.statusCode == 200){
            if(result == 'set phases to given value'){
            } else {
              self.log.info('Could not perform setNumPhases()')
            }
          } else {
            self.log.info("Check IP!")
            //self.stop();
    ...
    async interruptCp(intVal){
        const url = 'http://' + this.config.ip + '/interruptCp?interruptCp='+intVal;
        const self = this;
        request({method: "GET", url}, function (error, response, result) {
            if(!error && response.statusCode == 200){
